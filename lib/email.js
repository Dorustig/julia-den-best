...
<body style="margin:0;padding:0;background:#faf4ea;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1F2937;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#faf4ea;padding:32px 16px;">
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:20px;padding:36px 32px;box-shadow:0 8px 32px rgba(161, 122, 94,0.08);">
        <tr><td>
          <div style="font-family:'Playfair Display',Georgia,serif;font-size:22px;color:#a17a5e;font-weight:700;margin-bottom:4px;">Julia Besten</div>
          ...
          ${ctaUrl ? `
            <div style="margin:28px 0 8px;text-align:center;">
              <a href="${ctaUrl}" style="display:inline-block;background:#a17a5e;color:#fff;text-decoration:none;padding:14px 28px;border-radius:12px;font-weight:600;font-size:15px;">${escapeHtml(ctaText || 'Naar dashboard')}</a>
            </div>
          ` : ''}
...
async function sendWelcomeEmail({ to, naam, magicLink, loginUrl, email, password }) {
  ...
  if (email && password) {
    const body = `
      ...
      <div style="background:#faf4ea;border:1px solid #d9bfa6;border-radius:12px;padding:16px 20px;margin:16px 0;font-family:'SF Mono',Menlo,monospace;font-size:14px;color:#5c4632;line-height:1.8;">
...
/**
 * Wekelijkse check-in herinnering. Zondagavond cron.
 * Vriendelijk, niet pushy — geen vermelding van "je moet".
 */
async function sendCheckInReminderEmail({ to, naam, weekNr, daysSinceLast }) {
  const firstName = (naam || '').split(' ')[0] || 'daar';
  const tijdszin = daysSinceLast == null
    ? 'Je allereerste check-in wacht — leuk begin van je reis.'
    : (daysSinceLast > 10
      ? `Het is alweer ${daysSinceLast} dagen geleden dat je incheckte — misschien tijd?`
      : 'Je wekelijkse moment om even stil te staan bij hoe het gaat.');
  const body = `
    <p>Hoi ${escapeHtml(firstName)},</p>
    <p>${tijdszin}</p>
    <p>Je check-in duurt maar 2 minuutjes en geeft Julia + jou het beste inzicht: hoe voel je je, hoe zijn je maten, en (als je wilt) je voortgangsfoto's.</p>
    <p>Je bent nu in <strong>week ${weekNr} van 16</strong> — elke check-in is een vinkje richting je doel. 🌸</p>
  `;
  return sendMail({
    to,
    subject: `📅 Tijd voor je week ${weekNr} check-in`,
    html: layout({
      title: 'Hoe ging je week?',
      bodyHtml: body,
      ctaText: 'Check-in invullen',
      ctaUrl: `${SITE_ORIGIN}/klant/checkin`,
      footerNote: 'Liever niet meer deze herinnering? Mail Julia even — dan zetten we hem uit voor jouw account.',
    }),
  });
}

/**
 * Dagelijkse check-in herinnering (rond 20:00 's avonds).
 * Korter dan de wekelijkse reminder — geen grote oproep, gewoon lief duwtje.
 */
async function sendDailyHabitReminderEmail({ to, naam }) {
  const firstName = (naam || '').split(' ')[0] || 'daar';
  const body = `
    <p>Hoi ${escapeHtml(firstName)},</p>
    <p>Kort duwtje vanavond — je dagelijkse check-in staat nog open.</p>
    <p>30 seconden is genoeg: tik je 4 vakjes aan (💧 water, 😴 slaap, 👣 stappen, 🏋 training) en schrijf 1 zin in je journal.</p>
    <p style="color:#6B7280;font-size:13px;">Consistentie is niet perfect — het is <em>laten zien dat je er bent</em>, elke dag een beetje. Je kan het. 💪</p>
  `;
  return sendMail({
    to,
    subject: '☀️ Even je dagelijkse check-in?',
    html: layout({
      title: 'Kort duwtje voor vanavond',
      bodyHtml: body,
      ctaText: 'Open mijn dashboard',
      ctaUrl: `${SITE_ORIGIN}/klant/start`,
      footerNote: 'Wil je deze dagelijkse reminder uitzetten? Laat het Julia even weten.',
    }),
  });
}
...
async function sendKlantNewMessageEmail({ to, klantNaam, messagePreview }) {
  ...
  const body = `
    ...
    <div style="background:#faf4ea;border-left:3px solid #a17a5e;padding:14px 18px;border-radius:8px;margin:16px 0;font-style:italic;color:#5c4632;white-space:pre-wrap;">${escapeHtml(preview)}${messagePreview.length > 300 ? '…' : ''}</div>
